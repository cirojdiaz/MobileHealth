...
def train(model, X, labels, train_mask, val_mask=None, test_mask=None, validate=True, test=False, init_lr=0.01, stoch=False, num_batches=20, early_stopping=True, early_stopping_patience=10, early_stopping_warmup=0, max_epochs=4950, lamb_beta1=0.9, lamb_beta2=0.999, lamb_eps=1e-6, lamb_wd=0.01, lookahead_k=5, lookahead_alpha=0.5, l1_lambda=0.01):
    ...
    for e in range(max_epochs):
        ...
        print(class_weights)
        # loss_function = torch.nn.CrossEntropyLoss(pos_weight=class_weights)
        loss_function = torch.nn.BCEWithLogitsLoss(pos_weight=class_weights)

        # Inputs: logits, target:labels
        loss = loss_function(logits[train_mask].squeeze().float(), labels[train_mask].float())
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                print(f"Epoch [{e}], Parameter: {name}, Gradient: {param.grad.mean()}")
