# -*- coding: utf-8 -*-
"""4 Variables Model ANN (All Variables).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1PKLUfstj-QWRzRJK4IdzrPQDXrROkwD4

# 4 Variable ANN Model - All Variables
Purpose:
- Predict one variable from four variables (many-to-one predictions)
- A separate model is made for each variable

Notes:
- NaN rows are removed
- downsampling is applied to minimize redundancy
- Final ANN model using batch prediction method instead of manual (same method as the three variable model from Sumer 2022)
- based off of the Summer 2022 3 vars model
...
def splitDataToFeaturesTargetValues(train_array, test_array, numOfPrevValues=3, targetIndex=1):
  ...
  for i in range(numOfPrevValues, len(train_array)):
    ...
    nextData_train.append(train_array[i][targetIndex]) # append label as it's only the row at the target colun
  ...
  print("This is shape of train features array: "), print(np.array(prevData_train).shape)
  print("This is shape of test features array: "), print(np.array(prevData_test).shape)

  prevData_train = np.array(prevData_train)
  print("This is shape of old train labels array: "), print(np.array(nextData_train).shape)
  nextData_train = np.array(nextData_train).reshape(len(nextData_train), 1)
  print("This is shape of new train labels array: "), print(np.array(nextData_train).shape)

  prevData_test = np.array(prevData_test)
  print("This is shape of old test labels array: "), print(np.array(nextData_test).shape)
  ...
  print("This is shape of new test labels array: "), print(np.array(nextData_test).shape)
...
# listOfVariables = ["ECG"] # Change this to use a different health variable
# listOfVariables_len = len(listOfVariables) - 1
...
# For if MinMaxScaler was used; change the column number/index to get the values for other health variables
predictions_reversed = ((predictions - 0.1) / (1 - 0.1)) * (case4_filteredData_max[3] - case4_filteredData_max[3]) + case4_filteredData_max[3]
...
"""##Artificial Neural Network Model for Using HR, SpO2, NBP (Mean), ECG to Predict SpO2"""

# Running the model for SpO2 - artificial neural network

data_train_features, data_train_label, data_test_features, data_test_label = splitDataToFeaturesTargetValues(data_train, data_test, numOfPrevValues=27, targetIndex=1)
...
# Predicting with the model for SpO2 - training set - ANN
...
# Predicting with the model for SpO2 - test set - ANN
...
"""##Artificial Neural Network Model for Using HR, SpO2, NBP (Mean), ECG to Predict NBP"""

# Running the model for NBP - artificial neural network

data_train_features, data_train_label, data_test_features, data_test_label = splitDataToFeaturesTargetValues(data_train, data_test, numOfPrevValues=27, targetIndex=2)
...
# Predicting with the model for NBP - training set - ANN
...
# Predicting with the model for NBP - test set - ANN
...
"""##Artificial Neural Network Model for Using HR, SpO2, NBP (Mean), ECG to Predict ECG"""

# Running the model for ECG - artificial neural network

data_train_features, data_train_label, data_test_features, data_test_label = splitDataToFeaturesTargetValues(data_train, data_test, numOfPrevValues=27, targetIndex=3)
...
# Predicting with the model for ECG - training set - ANN
...
# Predicting with the model for ECG - test set - ANN
