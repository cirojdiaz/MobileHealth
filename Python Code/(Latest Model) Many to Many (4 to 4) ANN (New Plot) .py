...
# # Latest Many to Many ANN Model 
# The is the latest ANN model for Fall 2022 Team
# 
# Purpose:
# -  This model takes values of heart rate (HR), oxygen saturation (SpO2), normal blood pressure (NBP) and electrocardiogram (ECG) data to predict the same four health metrics simultaneously. 
# 
# 
# Notes:
# - Data was downsampled by 15 to minimize redundancy.
# - Dr. Ciro suggested to build a Many to Many model so it can be applicable in the app.
# - We use previous values to predict future ones, these future values will eventually become previous values.
#  
...
# ##Artificial Neural Network Model for Using HR, SpO2, NBP (Mean), ECG to Predict All 4 simultaneously
...
data_train_features, data_train_label, data_test_features, data_test_label = splitDataToFeaturesTargetValues(data_train, data_test, numOfPrevValues=500)
...
# Predicting with the model for All 4 metrics - training set - ANN
...
# Predicting with the model for All 4 metrics - test set - ANN ---------------- NEW METHOD
